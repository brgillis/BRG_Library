...
public:
	...
	void clear()
	{
		brgastro::set_zero(_test_run_points_);
		brgastro::set_zero(_means_);
		brgastro::set_zero(_best_fits_);
		brgastro::set_zero(_covars_);
		brgastro::set_zero(_stddevs_);
	}
			...
			const size_type & skip_factor=10)
	{
		...
		for(int_type step = 0; step < max_steps; step++)
		{
			...
			if(second_cycle)
			{
				if(brgastro::divisible(step,skip_factor))
				{
