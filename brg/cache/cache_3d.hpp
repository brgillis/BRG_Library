...
private:
	...
	const int _calc( const bool silent = false ) const
	{
				 ...
				 ( SPCP(name)->_max_3_ <= SPCP(name)->_min_3_ ) || ( SPCP(name)->_step_3_ <= 0 ) )
		{
			if ( !silent )
				std::cerr
						<< "ERROR: Bad range passed to brg_cache_3d::_calc() for " + SPCP(name)->_name_base() + "\n";
		...
		for ( unsigned int i_1 = 0; i_1 < SPCP(name)->_resolution_1_; ++i_1 )
		{
			...
			for( unsigned int i_2 = 0; i_2 < SPCP(name)->_resolution_2_; ++i_2)
			{
				...
				for( unsigned int i_3 = 0; i_3 < SPCP(name)->_resolution_3_; ++i_3)
				{
					...
					double result = 0;
					if(SPCP(name)->_calculate(x_1, x_2, x_3, result))
					{
						bad_result = true;
					}
...
public:
			...
			const bool silent = false ) const
	{
		...
		if ( !SPCP(name)->_loaded_ )
		{
			...
			if ( result == -1 )
			{
				std::string err = "ERROR: Could neither load " + SPCP(name)->_file_name_ + " nor calculate in brg_cache_3d::get()\n";
				if ( !silent )
					std::cerr << err;
