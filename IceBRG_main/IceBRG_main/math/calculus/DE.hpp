...
template< typename f, typename Td=flt_t, typename Tv=flt_t, typename Tt=flt_t >
inline const int_t leapfrog_step( const Td & x, const Td & y,
...
template< typename f, typename Td=flt_t, typename Tv=flt_t, typename Tt=flt_t >
inline const int_t leapfrog_step( Td & x, Td & y, Td & z,
		Tv & vx, Tv & vy, Tv & vz,
		const Tt & t_step, const f *accel_func )
{
	static_assert(std::is_same<decltype(Td()/Tt()),decltype(Tv())>::value,"Invalid types passed to parameters of leapfrog_step");
...
template< typename f, typename Tp, typename Tt=flt_t >
inline const int_t leapfrog_step( const Tp &p, Tp & new_p,
		const Tt & t_step, const f *accel_func )
{
...
template< typename f, typename Tp, typename Tt=flt_t >
inline const int_t leapfrog_step( Tp & p, const Tt & t_step,
		const f *accel_func )
{
	static_assert(std::is_same<decltype(p.x/Tt()),decltype(p.vx)>::value,"Invalid types passed to parameters of leapfrog_step");
