...
#define DECLARE_BRG_CACHE_3D(class_name,name_base,Tin1,Tin2,Tin3,Tout) \
class class_name : public IceBRG::brg_cache_3d<class_name,Tin1,Tin2,Tin3,Tout> \
{ \
private: \
 \
	...
	static IceBRG::array_t<IceBRG::array_t<IceBRG::array_t<Tout>>> _results_; \
 \
...
template<typename name, typename Tin1=flt_t, typename Tin2=flt_t, typename Tin3=flt_t, typename Tout=flt_t>
class brg_cache_3d
{
private:
	...
	void _load() const
	{
		...
		do
		{
			...
			// Initialise
			decltype(value_of(SPCP(name)->_results_[0][0][0])) temp_out;
			const std::streamsize out_size = sizeof(temp_out);
			ssize_t i_1=0, i_2=0, i_3=0;

			while ( ( !in_file.eof() ) && ( i_3 < SPCP(name)->_resolution_3_ )
					&& (in_file) )
			{
				in_file.read((char *)&temp_out,out_size);
				SPCP(name)->_results_[i_1][i_2][i_3] = units_cast<Tout>(temp_out);
	...
	void _output() const
	{
		...
		{
			temp_out = value_of(SPCP(name)->_results_[i_1][i_2][i_3]);
...
protected:
	...
	/// child.
	flt_t _calculate(const flt_t & x_1, const flt_t & x_2, const flt_t & x_3) const;
...
public:
	...
	void print( otype & out) const
	{
		...
		// Fill up data
		std::vector< std::vector<str_t> > data(5);
		std::stringstream ss;
		for(ssize_t i_1=0; i_1<SPCP(name)->_resolution_1_; ++i_1)
		{
			for(ssize_t i_2=0; i_2<SPCP(name)->_resolution_2_; ++i_2)
			...
			const Tin3 & x_3) const
	{
		...
		weighted_result = SPCP(name)->_results_[xi_1][xi_2][xi_3] * (xhi_1-x_1)*(xhi_2-x_2)*(xhi_3-x_3);
		weighted_result += SPCP(name)->_results_[xi_1+1][xi_2][xi_3] * (x_1-xlo_1)*(xhi_2-x_2)*(xhi_3-x_3);

		weighted_result += SPCP(name)->_results_[xi_1][xi_2+1][xi_3] * (xhi_1-x_1)*(x_2-xlo_2)*(xhi_3-x_3);
		...
		weighted_result += SPCP(name)->_results_[xi_1][xi_2+1][xi_3+1] * (xhi_1-x_1)*(x_2-xlo_2)*(x_3-xlo_3);
		weighted_result += SPCP(name)->_results_[xi_1+1][xi_2+1][xi_3+1] * (x_1-xlo_1)*(x_2-xlo_2)*(x_3-xlo_3);
